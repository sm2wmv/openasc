...
void write_data(unsigned char data) {
	...
	outb(LCD_DATA_POUT,temp);
}

unsigned char read_data(void) {
	unsigned char data = inb(LCD_DATA_PIN);
	unsigned char temp=0;
...
void lcdInitHW(void)
{
	...
	// initialize LCD data lines to pull-up
		...
		write_data(read_data() |0xF0);	// set pull-ups to on (4bit)
	#else
		outb(LCD_DATA_DDR, 0x00);						// set data I/O lines to input (8bit)
		write_data(0xFF);						// set pull-ups to on (8bit)
	#endif
#else
...
// write the control byte to the display controller
	...
	cbi(LCD_CTRL_PORT, LCD_CTRL_RW);			// set R/W to "write"
		...
		write_data((read_data() &0x0F) | (data&0xF0) );	// output data, high 4 bits
		LCD_DELAY;								// wait
		LCD_DELAY;								// wait
		cbi(LCD_CTRL_PORT, LCD_CTRL_E);	// clear "E" line
		LCD_DELAY;								// wait
		LCD_DELAY;								// wait
		sbi(LCD_CTRL_PORT, LCD_CTRL_E);	// set "E" line
		write_data((read_data()&0x0F) | (data<<4) );	// output data, low 4 bits
		LCD_DELAY;								// wait
		LCD_DELAY;								// wait
		cbi(LCD_CTRL_PORT, LCD_CTRL_E);	// clear "E" line
	#else
...
// read the control byte from the display controller
	...
	sbi(LCD_CTRL_PORT, LCD_CTRL_RW);		// set R/W to "read"
		...
		LCD_DELAY;						// wait
		LCD_DELAY;						// wait
		data = read_data()&0xF0;	// input data, high 4 bits
		cbi(LCD_CTRL_PORT, LCD_CTRL_E);	// clear "E" line
		LCD_DELAY;						// wait
		LCD_DELAY;						// wait
		sbi(LCD_CTRL_PORT, LCD_CTRL_E);	// set "E" line
		LCD_DELAY;						// wait
		LCD_DELAY;						// wait
		data |= read_data()>>4;	// input data, low 4 bits
		cbi(LCD_CTRL_PORT, LCD_CTRL_E);	// clear "E" line
	#else
		// 8 bit read
		sbi(LCD_CTRL_PORT, LCD_CTRL_E);	// set "E" line
		LCD_DELAY;						// wait
		LCD_DELAY;						// wait
		data = read_data();		// input data, 8bits
		cbi(LCD_CTRL_PORT, LCD_CTRL_E);	// clear "E" line
	#endif
	//	leave data lines in input mode so they can be most easily used for other purposes
#else
...
// write a data byte to the display
	...
	cbi(LCD_CTRL_PORT, LCD_CTRL_RW);		// set R/W to "write"
		...
		sbi(LCD_CTRL_PORT, LCD_CTRL_E);	// set "E" line
		outb(LCD_DATA_DDR, 0xFF);			// set data I/O lines to output (8bit)
		write_data(data);			// output data, 8bits
		LCD_DELAY;								// wait
		LCD_DELAY;								// wait
		cbi(LCD_CTRL_PORT, LCD_CTRL_E);	// clear "E" line
	#endif
