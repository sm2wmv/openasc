...
static const int adc_mapping[ROTATOR_COUNT] = {1, 0, 3};
...
int8_t bsp_rotator_is_running(uint8_t rot_idx) {
  ...
  if (rot_idx == 0)
    is_running = PORTC & ((1<<5) | (1<<4));
  else if (rot_idx == 1)
    is_running = PORTC & ((1<<3) | (1<<2));
  else if (rot_idx == 2)
    is_running = PORTC & ((1<<1) | (1<<0));
